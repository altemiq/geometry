...
public class NdxReaderTests
{
    ...
    public async Task Read()
    {
        await Assert.That(GetReader("test.ndx"))
            ...
            .Satisfies(reader => reader!.TotalNoOfPages, totalNoOfPages => totalNoOfPages.IsEqualTo(2)).And
            .Satisfies(reader => reader!.KeyLength, keyLength => keyLength.IsEqualTo((short)8)).And
            .Satisfies(
                reader => reader!.Read(),
                page =>
                    page
                        .IsNotNull().And
                        .Satisfies(p => p!.Count, count => count.IsEqualTo(3)).And
                        .Satisfies(p => p![0].ReadInt32(), i => i.IsEqualTo(1)).And
                        .Satisfies(p => p![1].ReadInt32(), i => i.IsEqualTo(2)).And
